...
contract FutureExchange {
    event TraderAccountCreation(address indexed trader, uint256 timestamp);
    ...
    event SettleContractsInvoked(uint256 timestamp);

    event Halted(uint256 timestamp, bool halted);
    ...
    event NewSlotStarted(uint256 timestamp);

    event FutureInitialPriceSet(uint256 timestamp, uint256 futurePrice);
    ...
    event UsdcClaimed(address trader, uint256 amount);

    event UsdcDeposited(address trader, uint256 amount);
    ...
    // timestamp => asset future price at that point
    struct FuturePrice {
        uint256 timestamp;
        uint256 assetPrice;
    }
    ...
    function createTraderAccount() public {
        ...
        require(
            USDCtoken.transferFrom(msg.sender, address(this), 10 * 10 ** 18),
    ...
    function buyAsset(uint256 _amount, uint8 _durationSlot) public {
        ...
        uint256 pricePaid = (currentFuturePrice *
            prePaymentPercentage *
            _amount) / 100;

        require(
            USDCtoken.transferFrom(msg.sender, address(this), pricePaid),
            "Insufficient USDC balance"
        );

        tradesCount[msg.sender]++;
        userContracts[msg.sender].push(
            Contract(Process.Buy, _amount, currentFuturePrice, block.timestamp)
        );
    ...
    function sellAsset(uint256 _amount, uint8 _durationSlot) public {
        ...
        tradesCount[msg.sender]++;
        userContracts[msg.sender].push(
            Contract(Process.Sell, _amount, currentFuturePrice, block.timestamp)
        );
    ...
    function settleAllContracts() public {
        ...
        for (uint i = 0; i < traderCount; i++) {
            ...
            if (netAssetsOwned[trader] < 0) {
                // sells more than buys
                int256 assetsToPurchase = netAssetsOwned[trader];

                int256 usdcToBePaid = (assetsToPurchase * (-1)) *
                    int256(futureValueAt[maturityTime]);

                int256 netFutureCumulative = futureCumulativeSum[trader] -
                    usdcToBePaid;

                if (netFutureCumulative < 0) {
                    // loss
                    uint256 positiveNetFutureCumulative = uint256(
                        netFutureCumulative * (-1)
                    );
                    // if(tradersSecurityAmount[trader] > positiveNetFutureCumulative) {
                    //     emit LessSecurityProvided!!()
                    // }
                    require(
                        tradersSecurityAmount[trader] >
                            positiveNetFutureCumulative,
                        "less security amount provided"
                    );
                    // all set. put the remaining tokens in the traders account
                    traderUSDCBalance[trader] +=
                        tradersSecurityAmount[trader] -
                        positiveNetFutureCumulative;
                } else {
                    // profit
                    // all set. put the remaining tokens in the traders account
                    uint256 amountToIncrease = tradersSecurityAmount[trader] +
                        uint256(netFutureCumulative);
                ...
                int256 usdcToReceive = assetsToSell *
                    int256(futureValueAt[maturityTime]);
                int256 netFutureCumulative = futureCumulativeSum[trader] +
                    usdcToReceive;
                if (netFutureCumulative < 0) {
    ...
    function updateFuturesPrice(uint8 _durationSlot) internal {
        ...
        int256 netDemand = int256(totalAssetsBought[maturityTime]) -
            int256(totalAssetsSold[maturityTime]);
        ...
        slotTotalTrades[maturityTime]++;
        futurePriceData[maturityTime].push(
            FuturePrice(block.timestamp, futureValueAt[maturityTime])
        );
    }

    function updateLastSettlementDate() internal {
    ...
    function claimUsdc(uint256 _amount) public {
        ...
        require(
            ...
            "INSUFFICIENT USDC IN TRADER ACCOUNT"
        );
        traderUSDCBalance[msg.sender] -= _amount;
    ...
    function getExecutionTimeBySlot(
        uint8 _durationSlot
    ) public view returns (uint256) {
        uint256 maturityTime = getLastSettlementDate() +
            (10 days) *
            (_durationSlot + 1);
        return maturityTime;
    }
